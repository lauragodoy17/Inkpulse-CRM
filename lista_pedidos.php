...
// Config de la barra de selección masiva por tp: a qué endpoint se manda,
// si el resultado se descarga (PDF) o solo cambia de estado, y los textos
// del botón/confirmación. Cubre todo el tramo "desde Aprobado en adelante".
$bulk_cfg = [
  3 => ['label' => 'Pasar a Procesando y generar planilla', 'icon' => 'bi-shuffle',
        'endpoint' => 'php/generar_planilla_procesamiento.php', 'download' => true,
  ...
  6 => ['label' => 'Pasar a Facturación', 'icon' => 'bi-file-earmark-text',
        'endpoint' => 'php/generar_lote_facturacion.php', 'download' => false,
  ...
  7 => ['label' => 'Pasar a En despacho', 'icon' => 'bi-box-seam',
        'endpoint' => 'php/generar_lote_despacho.php', 'download' => false,
  ...
  8 => ['label' => 'Marcar como Entregado', 'icon' => 'bi-truck',
        'endpoint' => 'php/generar_lote_entrega.php', 'download' => false,
...
// tp=7 (Facturación): cuántos pedidos quedaron guardados en lotes_facturacion
// (creados al pasar de Procesando a Facturación) que todavía no se han
// enviado por correo, para mostrar el botón "Enviar por correo".
$pend_facturacion = 0;
if ($tp == 7) {
  ...
  $pend_facturacion = intval($bdd->query(
    "SELECT COUNT(*) FROM lotes_facturacion_pedidos lp
     JOIN lotes_facturacion l ON l.id = lp.id_lote
     WHERE l.enviado = 0"
  )->fetchColumn());
}
?>
...
$(document).ready(function () {
  ...
  columns.push({
    ...
    render: function (data, type, row) {
      ...
      return '<div class="lp-acciones-cell">' + chk +
        '<a href="pedido_colegio.php?id_pedido=' + row.id + '&tp=' + TP + '" class="lm-btn-ver">' +
  ...
  $('#lp-btn-procesar').on('click', function () {
    ...
    confirmar(bulkCfg.confirm_title, ids.length + ' pedido(s). ' + bulkCfg.confirm_text, function () {
      // Con descarga (PDF): target="_blank" para que esta página nunca navegue
      // y quede libre para refrescar la tabla de inmediato. Sin descarga: se
  ...
  $('#lf-btn-enviar').on('click', function () {
    confirmar('¿Enviar por correo?', 'Se enviarán a facturacion3@somoseureka.com.co los pedidos pendientes.', function () {
      var $form = $('<form>', { method: 'POST', action: 'php/enviar_lote_facturacion.php' });
      $('body').append($form);
      $form.submit();
    });
  });
