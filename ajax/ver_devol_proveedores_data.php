<?php
/**
 * Fuente de datos server-side (DataTables) para ver_devol_proveedores.php.
 * Recibe draw/start/length/search/order (protocolo estándar de DataTables)
 * y resuelve todo en SQL para no traer más filas de las que se van a pintar.
 */
...
$extra = [];
if ($search_val !== '') {
  $extra[] = "(p.id LIKE :s OR CONCAT(u.nombres,' ',u.apellidos) LIKE :s OR c.proveedor LIKE :s OR e.estado LIKE :s)";
  $params[':s'] = '%' . $search_val . '%';
}
...
$order_map     = ['id' => 'p.id', 'fecha_d' => 'p.fecha', 'proveedor' => 'c.proveedor', 'estado' => 'e.estado'];
...
$sql = "SELECT p.id, p.tipo, p.fecha, u.nombres, u.apellidos, e.estado, c.proveedor AS cliente
        $from $where_f
...
foreach ($rows as $p) {
  $data[] = [
    'id'        => $p['id'],
    'tipo'      => $p['tipo'],
    'fecha_d'   => htmlspecialchars($p['fecha']),
    'usuario'   => htmlspecialchars(trim(($p['nombres'] ?? '') . ' ' . ($p['apellidos'] ?? ''))),
    'proveedor' => htmlspecialchars($p['cliente'] ?? ''),
